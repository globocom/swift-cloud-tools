# EXAMPLE
# python scripts/pages/1_create_sp_qa_project_bucket.py fee525a415c44147896903fab66d6855 alanvitor False development prod
...
cnx_vault = mysql.connector.connect(
    ...
    host=os.environ.get("MYSQL_VAULT_HOST"),
    database=os.environ.get("MYSQL_VAULT_NAME")
)
cursor_vault = cnx_vault.cursor(buffered=True)
...
if applying:
    ...
    try:
        ...
        project = conn.projects.get(legacy_swift_id)
        ...
        if team_owner_id:
            sql = "select name " \
                  "from auth_group " \
                  "where id = %s;" % team_owner_id

            query = (sql)
            cursor_vault.execute(query)

            team_name = cursor_vault.fetchone()[0]
